...
详细启动、关闭与请求格式：docs/BROKER-RUNBOOK.md`);
    ...
    const connection = connectionFile ? JSON.parse(await readFile(path.resolve(connectionFile), 'utf8')) : {
      baseUrl: `http://${settings.host === '::1' ? '[::1]' : settings.host}:${settings.port}`, credentialsFile: path.join(settings.dataDir, 'credentials.json') };
    const base = new URL(connection.baseUrl);
    if (base.protocol !== 'http:' || !['localhost', '127.0.0.1', '[::1]'].includes(base.hostname) || base.username || base.password || base.pathname !== '/' || base.search || base.hash) throw new Error('作战室CLI仅连接本机HTTP入口');
