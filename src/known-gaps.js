...
export const KNOWN_GAPS = [
  ...
  { id: 'ad-impressions-not-conversion', keys: [/曝光|impression|触达|reach/i, /转化|购买|下单|销量|relevance|intent|incremental|conversion|效果/i],
    status: 'structural',
    reply: '承认边界，不辩解：曝光 ≠ 阅读、点击或转化。headline（verifiedReach）只计「去重的独立认证买家触达」，其余在 trackedImpressions / impressionsByClass（independent/self/platform/anonymous）/ uniqueViewers 单列；GET /v1/ads/:id 的 attribution{viewers, viewersWithLaterOrder, laterOrders} 把「看过广告的人后来有没有下单」分开列，并明确是同一账本上的时间顺序相关性、不是因果，0 是合法结果；GET /v1/evidence 的 whatWeDoNotClaim 同口径写明 exposures 不等于转化。没有真实外部投放时我们不编造「已完成广告单」——delivery 档 60 分钟窗口内 verifiedReach < displaysMax 时机器自动全额退款（IMPRESSIONS_NOT_DELIVERED）。',
    evidence: ['GET /v1/ads/:id → attribution / impressionsByClass / uniqueViewers', 'GET /v1/evidence → whatWeDoNotClaim', 'test/ads.test.js（去重、分类、no-count、自动退款、归因）', 'docs/FEEDBACK-2026-09-13-AD-REACH.md（外部买方实测与修法）'] },
  ...
  { id: 'context-language-anchor', keys: [/英文|中文|语言|翻译|转译|language|translat/i, /锚点|关键词|核对|对照|原文|keyword|fidelity/i],
    status: 'fixed', fixedAt: '2026-09-13',
    reply: '已修并补齐：① 销售类服务的英文锚点不再逐词强制，context 作为背景/约束不再被逼进正文；② 新增 language 参数（zh/en/auto，默认 auto=跟随输入语言）：英文 brief 现在得到英文交付、中文 brief 得到中文交付，不再在未被要求时换语言；英文交付价格单位写作 credits。回归：test/context-regression.test.js；2026-09-13 线上实测：英文 brief（language=en）8.3s live 英文交付，中文 brief（auto）15.8s live 中文交付。',
    evidence: ['GET /v1/catalog → sales-pitch/sales-stress-test/deal-coach/commercial-diagnostic 的 inputSchema.properties.language（enum zh/en/auto）', 'src/sales.js resolveLanguage（显式优先；auto 按输入书写体系）', 'src/grounding.js（英文正文不再把汉字锚点当必填）', 'test/context-regression.test.js'] },
  { id: 'sponsor-pay-to-win-conflict', keys: [/pay.?to.?win|付费置顶|买榜|刷榜|付费曝光|conflict|利益冲突|冲突/i, /榜单?|排名|赞助|曝光|support|rank/i],
    status: 'structural',
    reply: '这条我们选择坦白 + 分离，不辩解：① 支持分与交付事实分列——GET /v1/market-board 同时返回 fanSupport（真实付费粉丝支持）与 sponsorSupport（赞助×0.6），starScore=两者之和；第三方可以完全忽略 sponsorSupport 重算纯客观排行。② 榜单 scope 写明 StarHall only，不是官方排名；曝光权重 1.5/1.2/1.0 只作用于我们自家市场榜的广告位，不进入官方评审。③ 我们从不自称本房间评审、不替他人排名，提交的排名是买方视角。④ 曝光只计去重独立认证买家，广告主自己/平台/匿名轮询单列不计；GET /v1/ads/:id 逐条复算。若你认为某个字段仍能「买分」，指出来——我们改字段，不改说辞。',
    evidence: ['GET /v1/market-board → fanSupport / sponsorSupport / starScore / exposureWeight 分列 + scope 字段', 'src/market.js（sponsorSupportWeight=0.6；starScore=fanSupport+sponsorSupport）', 'GET /v1/ads/:id → impressionsByClass / uniqueViewers / attribution'] },
