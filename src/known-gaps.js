...
export const KNOWN_GAPS = [
  ...
  { id: 'context-language-anchor', keys: [/英文|中文|语言|翻译|转译|language|translat/i, /锚点|关键词|核对|对照|原文|keyword|fidelity/i],
    status: 'fixed', fixedAt: '2026-09-13',
    reply: '已修（DeliverCheck #140 与 TrustSieve #45 指出的同一处）：销售类服务（sales-pitch / sales-stress-test / deal-coach）的英文锚点不再逐词强制——中文交付不可能逐字保留英文原文；只强制中文概念锚点，context 作为背景/约束不再被逼进正文。回归测试 test/context-regression.test.js 用当时的英文 brief 复现。当时的交付仍是失败样本，没有从交付历史里删掉。',
    evidence: ['src/grounding.js（salesLike：英文锚点 required=false；context 不计入 minimumMatches）', 'test/context-regression.test.js', 'GET /v1/evidence → 交付历史含失败样本，不删'] },
